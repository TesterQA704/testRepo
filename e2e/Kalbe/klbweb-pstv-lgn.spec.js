...
test.describe('Test Login Website2 Kalbe', () => {
    test('amon positive login', async ({ page }) => {
        ...
        await page.getByLabel('firstname.lastname@compdomain').fill(credentials.username);
    ...
    test('bsCorp positive login', async ({ page }) => {
        ...
        await page.getByPlaceholder('firstname.lastname@compdomain').fill(credentials.username);
    ...
    test('ils positive login', async ({ page }) => {
        ...
        await page.getByPlaceholder('Your Password').fill(credentials.password);
        await page.getByRole('button', { name: ' LOG IN' }).click();
    ...
    test('kation positive login', async ({ page }) => {
        ...
        await page.getByPlaceholder('user.domain@kalbe.co.id').fill(credentials.username);
        await page.getByPlaceholder('Password User Domain Onekalbe').fill(credentials.password);
        await page.getByRole('button', { name: 'Login' }).click();
    ...
    test('momonline positive login', async ({ page }) => {
        ...
        await page.getByPlaceholder('login menggunakan user').fill(credentials.username);
        await page.getByPlaceholder('password').fill(credentials.password);
        await page.getByRole('button', { name: '   Login' }).click();
    ...
    test('PLM positive login', async ({ page }) => {
        ...
        await page.getByPlaceholder('ex: user.plm@kalbe.co.id').fill(credentials.username);
